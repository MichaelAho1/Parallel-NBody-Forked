...
def graph_overhead_comparison(df: pd.DataFrame, ns_focus: list = None, save: bool = True) -> None:
    ...
    if "treebuild_oh_pct_avg" not in df.columns or "forces_oh_pct_avg" not in df.columns:
        print("  skipping overhead comparison: treebuild_oh_pct_avg / forces_oh_pct_avg columns not found")
        return
    ...
    for n in ns_focus:
        ...
        tb_oh  = pd.to_numeric(sub["treebuild_oh_pct_avg"], errors="coerce").tolist()
        f_oh   = pd.to_numeric(sub["forces_oh_pct_avg"],    errors="coerce").tolist()
        ...
        ax.set_xticklabels([str(t) for t in threads])
        ax.set_xlabel("Number of Threads")
        ax.set_ylabel("Overhead as % of Phase Time")
